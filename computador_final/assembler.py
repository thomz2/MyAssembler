...
instructions = ['add', 'sub', 'goto', 'mov', 'jz', 'halt', 'wb', 'ww', 'mult', 'smlr', 'div', 'mod', 'min', 'ig', 'bigr', 'desl1', 'desl2']
instruction_set = {'halt' : 0xFF, # 255 
                   'add'  : 0x02, # 2
                   'mov'  : 0x05, # 5
                   'goto' : 0x08, # 8
                   'sub'  : 0x0A, # 10
...
def encode_desl(inst):
   line_bin = []
   line_bin.append(instruction_set[inst])
   return line_bin
      
def encode_instruction(inst, ops):
   if inst == 'add' or inst == 'sub' or inst == 'mov' or inst == 'jz' or inst == 'mult' or inst == 'smlr' or inst == 'div' or inst == 'mod' or inst == 'min' or inst == 'ig' or inst == 'bigr':
   ...
   elif inst == 'desl1' or 'desl2':
      return encode_desl(inst)
   else:
      return []
...
def resolve_names():
   ...
   for line in lines_bin:
      for i in range(0, len(line)):
         if is_name(line[i]):
            if line[i-1] == instruction_set['add'] or line[i-1] == instruction_set['sub'] or line[i-1] == instruction_set['mov'] or line[i-1] == instruction_set['mult'] or line[i-1] == instruction_set['smlr'] or line[i-1] == instruction_set['div'] or line[i-1] == instruction_set['mod'] or line[i-1] == instruction_set['min'] or line[i-1] == instruction_set['ig'] or line[i-1] == instruction_set['bigr']:
